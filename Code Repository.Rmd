...
ggplot(data = null_normal_distribution.df, aes(x =null_normal_distribution )) + geom_histogram(bins =250, colour = "skyblue", fill = "skyblue") + labs(title = "Normal Null Distribution", x = "", y = "Frequency") +
  ...
  geom_vline( xintercept = study_summed_composite, color = "gold", size = 1 ) + 
  annotate("text", x = study_summed_composite, y = 10000, label = "EMERGE Composite", color = "gold", angle = 90, vjust = -0.5)
```
...
## Latent Variable Analysis McMenamin et al.
The following code is from McMenamin et al. 2021 which has been modified to remove the code for baseline variables of the continuous outcomes.
```{r}
#Likelihood function
...
results <- LatVarfunc(inverted_data, inverted_eta)
names(results) <- c("logOR_LCI", "logOR", "logOR_UCI")
```

Power estimation of Latent variable analysis
*Note that this takes a long time to run, and as such we only ran it for two sample sizes, 100 and 150*
```{r}
set.seed(24253739)
power_function_null_dist <- function(n) {
...
sample_vector <- 100
##null_dist_summed_TS_results <- sapply(sample_vector, function(n_value) {power_function_null_dist(n_value)})
...
type_1_error_rate_100 <- 0.49
type_1_error_rate_200 <- 0.57
```
...
power_function <- function(x, y, z, e, n) {
  summed_TS_power_result <- numeric(1000)  
  squared_TS_power_result <- numeric(1000)  
  EMERGE_composite_power <- numeric(1000)
  i <- 1
  repeat {
    ...
    i<- i +1
    if(i > 1000) break}
  return(as.numeric(c(sum(summed_TS_power_result<quantile(permuted_null_distributions.df$permutation_summed_composite, 0.025)| summed_TS_power_result > quantile(permuted_null_distributions.df$permutation_summed_composite, 0.975))/ 1000,## permutation test summed
         sum(squared_TS_power_result>quantile(permuted_null_distributions.df$permutation_squared_composite, 0.95))/ 1000, ##permutation test squared
         sum(squared_TS_power_result > quantile(theoretical_null_squared$chi_sq, 0.95))/ 1000, ##asymptotic chi-square
         sum((EMERGE_composite_power< 0.05))/ 1000, ## EMERGE
         sum(summed_TS_power_result<quantile(null_normal_distribution, 0.025)| summed_TS_power_result > quantile(null_normal_distribution, 0.975))/ 1000)))
}
```
