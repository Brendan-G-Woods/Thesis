...
Composites are commonly used in clinical trials as the primary outcome. When misused, they can reduce the power to detect treatment effect. This was seen in EMERGE, a randomised clinical trial investigating the effects of early initiation of Metformin treatment on developing gestational diabetes. The composite was created using three measures of glycemic control; blood glucose levels of 5.1mmol/L or greater at gestation weeks 32 or 38, or initiation of Insulin treatment. 

 ***Insulin~Yes~ OR gw32≥5.1~Yes~ OR gw38≥5.1~Yes~ ***

This composite involves dichotomising blood glucose levels, greatly reducing power. Therefore, the objectives of this project are:

1.	To perform a meta-analysis of the primary outcome using permutations of the composite test statistic, and analyzing the glucose components as continuous variables

2.	To compare various ways of creating the composite test statistic, and their power

3.	To compare the permutation method with other approaches to analyzing composite outcomes
...
ggplot(squared_composite.df, aes(x= squared_composite))+
  geom_histogram(bins = 200, fill = "salmon", color = "salmon")+
...
* Compare the permutation test method with the approach Mc Menamin *et al.*, [^5] used to analyze composite outputs.
